...
if [[ "$PLUGINS_EXTRA" == "true" ]]; then
  for plug in netcfgplugin sqlplugin processes ; do
    ...
    git clone --depth 1 https://github.com/deep-soft/far2m-$plug.git && \
    ( cd $plug && \
...
if [[ "$STANDALONE" == "true" ]]; then
  ( cd $INSTALL_DIR && ./far2m --help >/dev/null && bash -x $REPO_DIR/make_standalone.sh ) && \
  makeself --keep-umask $REPO_DIR/far2m/$BUILD_DIR/$INSTALL_DIR $PKG_NAME.run "FAR2M File Manager" ./far2m && \
...
if [[ "$APPIMAGE" == "true" ]]; then
  ...
  export ARCH=$(uname -m)
  ( cd $REPO_DIR && \
    AppDir/usr/bin/far2m --help >/dev/null && \
    sed 's|@APP@|far2m|' -i AppRun && chmod +x AppRun && \
